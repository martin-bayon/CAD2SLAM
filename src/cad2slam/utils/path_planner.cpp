...
namespace cad2slam {
  ...
  std::vector<Vector2i> PathPlanner::doPathPlanning(Vector2i start_pixel, Vector2i goal_pixel, std::string yaml_filename){
    loadMap(yaml_filename);
    /*
    {
      ...
      cv::imshow("parent map before", p);
      cv::imshow("distance map before", m);
      cv::waitKey();
    }
    */
    ...
    auto curr_robot_world = grid2world(start_pixel.y(), start_pixel.x());
    ...
    auto goal_world = grid2world(goal_pixel.y(), goal_pixel.x());
  ...
  PathMatrixCostSearch::SearchStatus PathPlanner::doDijkstra(const Vector2i& goal, const Vector2i& start, PathMatrix& pmap) {
  ...
  inline float PathPlanner::getCost(const Vector2i& pos, const PathMatrix& m) {
    if (! m.inside(pos))
      return -1;
    return m.at(pos).cost;
  }

  inline bool PathPlanner::valueAndGradient(Vector3f& dest_, const Vector2i& pos_, const PathMatrix& matrix_, const float& gain) {
  ...
  inline bool PathPlanner::getCostAtSubPixel(Vector3f& dest_, const Vector2f& pos_, const PathMatrix& matrix_, const float& gain) {
  ...
  inline Eigen::Vector2f PathPlanner::grid2world(int r, int c) {
    assert(_grid_map && "grid map not set");
    return _grid_map->indices2global(Vector2i(r, c));
  }

  inline Eigen::Vector2i PathPlanner::world2grid(const Eigen::Vector2f p) {
    assert(_grid_map && "grid map not set");
    return _grid_map->global2indices(p);
  }

  float PathPlanner::computePathGrid(StdVectorEigenVector3f& path_, const Vector3f& current_pose_, PathMatrix& pm_, const GridMap2DHeader& mh_) {
    path_.clear();
    ...
    while (steps < max_steps) {
      ...
      _px_path.push_back(Vector2i(current_idx.y(), current_idx.x()));
      ++steps;
      if (pm_.at(current_idx).cost == 0) {
        //std::cerr << " -- curr_idx.cost: " << pm_.at(current_idx).cost << " -- return" std::endl;
        return length;
      }else{
  ...
  float PathPlanner::computePathGradient(StdVectorEigenVector3f& path_, const Vector3f& current_pose, PathMatrix& pm_, const GridMap2DHeader& mh_) {
    path_.clear();
  ...
  void PathPlanner::setMap(GridMap2DPtr grid_map_) {
  ...
  bool PathPlanner::loadMap(const std::string& yaml_filename) {
    ...
    if (resolution == 0 || !image_filename.length()) {
      //std::cerr << "Returning a nullptr at 2" << std::endl;
      return false;
    }


    int pos = yaml_filename.find_last_of('/');
    image_path = yaml_filename.substr(0, pos+1) + image_filename;

    cv::Mat cv_img = cv::imread(image_path, CV_LOAD_IMAGE_ANYDEPTH | CV_LOAD_IMAGE_ANYCOLOR);
    cad_cv = cv_img.clone();
  ...
  bool PathPlanner::computePolicy(const Vector3f& goal) {
